...
class BildMaschine():
    ...
    def RevAnfangenWash(self):
        ...
        s_row=0+Shift
        s_col=QueryW-self.LetterSpace
        
        BaseLine=0 #the baseline is the line of the last char
        CharCount=0
        ...
        for c in self.Query:
            ...
            for gl in self.BilderBuch:
                if str(c) in gl.Char: #find the char in the glyphbook
                    ...
                    self.RevConcatWash(s_col, s_row, ImgQuery, gl.Naher,gl.Top,gl.Low,gl.Left,gl.Right,e)
                    e=e+1
                    
                    #!!!!
                    s_col-=+self.LetterSpace+gl.Left-gl.StartLeft
    ...
    def RevConcatWash(self,StartCol,StartRow,qpic_arr,char_arr,top,low,left,right,e):
        #concatenate chars
        print 'Copying from col',left,'to',right
        print 'Copying from row',top,'to',low
        ...
        #and pastes them to final image query
        for i in reversed(range(0, low-top)):
            for j in reversed(range(0, right-left)):
                qpic_arr[i+StartRow][j-StartCol]=clean_char_arr[i][j]
